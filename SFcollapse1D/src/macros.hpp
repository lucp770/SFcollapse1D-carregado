...
/* for a kiselev spacetime (whith anisotropic fluid) we have the metric
ds^2 = -f(r)dt^2 + dr^2/f(r) + r^2*dO^2
where:
f(r) = 1 - 2M/r -c/(r^(3w_q+1)) 

#for a cosmological constant spacetime we have the metric
f(r) = 1 - 2M/r - (COSMOLOGICAL_CONSTANT)*r^2;
*/
...
#define DECLARE_GRID_PARAMETERS                                                       \
  ...
  const real w_q                   __attribute__((unused))  = W;                         \
  const real cosmological_constant __attribute__((unused))  = COSMOLOGICAL_CONSTANT;     \
  const real c_final               __attribute__((unused))  = C;                         \
  const real anisotropic_exponent  __attribute__((unused))  = 2.0 + 3.0 * w_q;
