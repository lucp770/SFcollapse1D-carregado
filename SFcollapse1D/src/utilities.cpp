...
/* extra includes for numeric checking */
...
void utilities::compute_and_output_mass_aspect_function( const int which_level, const int n, const grid::parameters grid, const gridfunction a ) {
  ...
  LOOP(0,Nx0Total) {
    ...
    // const  double l2 = 3/COSMOLOGICAL_CONSTANT;
    const real mass    = 0.5 * r_ito_x0[j] * ( 1.0 - 1.0/SQR(a_local) - c_final/(pow(r_ito_x0[j], 3 * w_q + 1)));
    outfile << scientific << x[0][j] << " " << r_ito_x0[j]  << " " << mass << endl;
  }
  outfile.close();
  //is necessary to implement the mass calculation for the cosmological constant alternatively.
  ////////////////////////////////////////////////////////////////////////////////////////////////////////
...
/* function templates for checking valid numbers */
template<typename T>
...
real utilities::random_search_positive_values(real inv_dx0, real A, real tmp0, real half_invr, real PhiPiTerm, real ans_fluid_term ){
  ...
  for(int i=0; i < 100000000; i++){
    ...
    if( result2 < 0){
      ...
      if (is_valid(result2)){
           ...
           if (is_inclination_positive(number2,inv_dx0, A, tmp0,half_invr,PhiPiTerm,ans_fluid_term)){
...
void utilities::generate_plot_for_bissection(real inv_dx0, real A, real tmp0, real half_invr, real PhiPiTerm, real ans_fluid_term ){
